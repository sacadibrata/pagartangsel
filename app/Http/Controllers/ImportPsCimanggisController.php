...
class ImportPsCimanggisController extends Controller
{
    public function importFormCimanggis()
    {
        $user = auth()->user();
        $pasars = DataSurveyor::where('user_id',$user->id)->first();
        return view('backend.pasar_cimanggis.import.form', compact('pasars'), ['title' => 'UPLOAD DATA']);
    }

    public function importDataCimanggis(Request $request)
    {
        // Validasi file Excel
        $request->validate([
        ...
        return redirect()->route('importFormCimanggis')->with('success', 'Data berhasil diimpor.');
    }
