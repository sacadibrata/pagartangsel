...
class ImportPsCiputatController extends Controller
{
    public function importFormCiputat()
    {
        $user = auth()->user();
        $pasars = DataSurveyor::where('user_id',$user->id)->first();
        return view('backend.pasar_ciputat.import.form', compact('pasars'), ['title' => 'UPLOAD DATA']);
    }

    public function importDataCiputat(Request $request)
    {
        // Validasi file Excel
        $request->validate([
        ...
        return redirect()->route('importFormCiputat')->with('success', 'Data berhasil diimpor.');
    }
