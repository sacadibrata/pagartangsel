...
class ImportPsJombangController extends Controller
{
    public function importFormJombang()
    {
        $user = auth()->user();
        $pasars = DataSurveyor::where('user_id',$user->id)->first();
        return view('backend.pasar_jombang.import.form', compact('pasars'), ['title' => 'UPLOAD DATA']);
    }

    public function importDataJombang(Request $request)
    {
        // Validasi file Excel
        $request->validate([
        ...
        return redirect()->route('importFormJombang')->with('success', 'Data berhasil diimpor.');
    }
