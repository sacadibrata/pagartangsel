...
class KomoditasSemuaPasar extends Model
{
    use HasFactory;
    ...
    {
        return $this->hasMany(GabunganData::class, 'komoditas_id');
    }
    ...
    public function hasInputToday()
    {
        ...
        return $this->pendataansemuapasar->where('tanggal_input', $today)->isNotEmpty();
    }

    public function hasInputYesterday()
    {
        // Ambil tanggal hari ini
        $yesterday = now()->subDay(55)->format('Y-m-d');

        // Periksa apakah ada input harga pada tanggal hari ini
        return $this->pendataansemuapasar->where('tanggal_input', $yesterday)->isNotEmpty();
    }
}
