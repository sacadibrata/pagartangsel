            ...
            <div class="d-md-flex justify-content-between align-items-center">
                <!-- pageheader -->
                    ...
                    <nav aria-label="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('pendataanAll.indexSemuaPasar') }}" class="text-inherit">List Price Product</a></li>
                    ...
                    <!-- button -->
                    <div>
                        <a href="{{ route('pendataanAll.indexSemuaPasar') }}" class="btn btn-outline-secondary mb-2">Back to List Price</a>
                    </div>
            </div>
        </div>
      </div>
                        ...
                        <tbody>
                            ...
                            @foreach($pendataan as $key => $data)
                                <tr>
                                    ...
                                    <td>{{ number_format($data->rataRataCeger, 0, ',', '.') }}</td>
                                    <td>{{ number_format($data->rataRataCiputat, 0, ',', '.') }}</td>
                                    <td>{{ number_format($data->rataRataSerpong, 0, ',', '.') }}</td>
                                    <td>{{ number_format($data->rataRataCimanggis, 0, ',', '.') }}</td>
                                    <td>{{ number_format($data->rataRataJombang, 0, ',', '.') }}</td>
                                    <td>{{ number_format($data->rataRataJengkol, 0, ',', '.') }}</td>
                                    <td>{{ number_format($data->rataRataSemuaPasar, 0, ',', '.')  }}</td>
                                    <td>
                                        @if ($key > 0)
                                        @php
                                            $hargaKemarin = $pendataan[$key - 1];
                                            $hargaHariIni = $data;
                                            $perbandingan = ($hargaHariIni->rataRataSemuaPasar > $hargaKemarin->rataRataSemuaPasar)
                                            ? 'Naik'
                                            : ($hargaHariIni->rataRataSemuaPasar < $hargaKemarin->rataRataSemuaPasar ? 'Turun' : 'Tetap');
                                        @endphp
                                    ...
                                    <td>
                                        <form action="{{ route('pendataanAll.destroySemuaPasar', $data->id_pendataan) }}" method="POST" style="display: inline-block;">
                                            @csrf
                                            @method('DELETE')
